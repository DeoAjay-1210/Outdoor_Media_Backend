...
// // ─────────────────────────────────────────────────────────────
// // VALIDATE LAND OWNER PAYMENT CATEGORY
// //
// // Rules:
// //   paymentCategory=1 (Cash)        → no extra fields required
// //   paymentCategory=2 (Online)      → onlineMode required
// //   paymentCategory=3 (Cash+Online) → onlineMode + cashAmount + onlineAmount
// //                                     cashAmount + onlineAmount must equal owner's share
// // ─────────────────────────────────────────────────────────────
// const validateOwnerPaymentCategories = (
//   landOwners,
//   rentalAmount // Amount before GST
// ) => {
...
//       // GST NOT INCLUDED IN OWNER SHARE CALCULATION
//       let ownerShare = 0;

//       if (Number(owner.typeShare) === 1) {
//         ownerShare = parseFloat(
//           (
//             (rentalAmount * (Number(owner.sharePercentage) || 0)) /
//             100
//           ).toFixed(2)
//         );
//       } else {
...
// //
// // Net payable = (totalRentalAmount + gstAmount) − tdsAmount
// // typeShare=1 → percentage of netPayable
// // typeShare=2 → fixed shareAmount
// // Sum must equal netPayable (±1 tolerance)
// // ─────────────────────────────────────────────────────────────
...
//   gstType
// ) => {
//   // Step 1 — GST-adjusted base
//   let baseAmount = totalRentalAmount;
//   if (gstApplicable === 1 && gstType === 2) {
//     const envGstPct = parseFloat(process.env.GST_PERCENTAGE || "18");
//     const gstAmount = parseFloat(((totalRentalAmount * envGstPct) / 100).toFixed(2));
//     baseAmount = parseFloat((totalRentalAmount ).toFixed(2));
//   }
 
//   // Step 2 — TDS on GST-adjusted base
//   const tdsAmount =
//     tdsApplicable === 1 && tdsPercentage > 0
//       ? parseFloat(((baseAmount * tdsPercentage) / 100).toFixed(2))
//       : 0;
 
//   const netPayable = parseFloat((baseAmount - tdsAmount).toFixed(2));
 
//   // Step 3 — Sum owner shares
...
//         };
//       }
//       totalComputedAmount += parseFloat(((netPayable * sharePercentage) / 100).toFixed(2));
//     } else if (typeShare === 2) {
...
//     return {
//       valid: false,
//       message: `Total owner share does not match net payable amount (${netPayable.toFixed(2)}) after GST and TDS deductions. Difference: ${diff.toFixed(2)}`,
//     };
//   }
 
//   return { valid: true, netPayable };
// };
...
//   if (gstApplicable === 1) {
//     const gstType = Number(rentalPayment?.gstType);
//     if (![1, 2].includes(gstType)) {
//       return {
//         valid: false,
//         message:
//           "rentalPayment.gstType is required and must be 1 (Inclusive) or 2 (Exclusive) when gstApplicable is 1",
//       };
//     }
...
//       }
//       if (mediaData.rentalPayment.gstType !== undefined) {
//         mediaData.rentalPayment.gstType = Number(mediaData.rentalPayment.gstType);
//       }
//     }
 
//     // ── Convert landOwners ─────────────────────────────────
//     // Using !== undefined/null/"" checks to preserve 0 as a valid value.
...
//         converted.cashAmount = hasValue(owner.cashAmount) ? Number(owner.cashAmount) : 0;
//         converted.onlineAmount = hasValue(owner.onlineAmount) ? Number(owner.onlineAmount) : 0;
 
//         return converted;
//       });
//     }
 
//     // ── KEY FIX: Auto-assign single owner full share ───────
//     // When numberOfLandOwners=1, typeShare is not required from the client.
//     // The single owner automatically gets 100% of netPayable.
//     if (mediaData.landOwners?.length === 1) {
//       const owner = mediaData.landOwners[0];
//       if (!owner.typeShare) {
//         owner.typeShare = 1;          // percentage-based
//         owner.sharePercentage = 100;  // 100% of netPayable
//       }
//     }
...
//     // ── VALIDATION: Land owner shares (GST-aware) ──────────
...
//       const gstApplicable = Number(mediaData.rentalPayment.gstApplicable) || 0;
//       const gstType = Number(mediaData.rentalPayment.gstType) || 0;
...
//           oldAppraisal.totalAppraisalAmount || 0,
//       });

//       oldAppraisal.lastAppraisalDate =
//         oldAppraisal.nextAppraisalDate;
//     }
...
//       // ── KEY FIX: Reload from DB to get all pre-save computed values ──
//       // Pre-save hooks mutate `this` (the Mongoose doc) but the in-memory
//       // object returned after .save() sometimes doesn't reflect nested
//       // mutations (e.g. rentalPayment.netPayable, ownerPayments).
//       // Reloading with findById guarantees the response matches what's in MongoDB.
//       media = await MediaOnboarding.findById(media._id).lean();
...
//
// Rules:
//   gstApplicable=1  → paymentCategory MUST be 2 (Online only)
//                      Cash (1) and Cash+Online (3) are NOT allowed
//   gstApplicable=0:
//     paymentCategory=1 (Cash)        → no extra fields required
//     paymentCategory=2 (Online)      → onlineMode required
//     paymentCategory=3 (Cash+Online) → onlineMode + cashAmount + onlineAmount
//                                       cashAmount + onlineAmount must equal owner's share
//
// ownerNetPayable = owner's computed share amount (from netPayable, post GST+TDS)
// ─────────────────────────────────────────────────────────────
const validateOwnerPaymentCategories = (
  landOwners,
  netPayable,       // total netPayable (totalRentalAmount + GST - TDS)
  gstApplicable     // 0 or 1
) => {
  for (const owner of landOwners) {
    const cat = Number(owner.paymentCategory);

    // ── NEW RULE: GST applicable → Online only ────────────
    if (gstApplicable === 1) {
      if (cat !== 2) {
        return {
          valid: false,
          message: `Owner "${owner.name}": When GST is applicable, paymentCategory must be 2 (Online) only. Cash (1) and Cash+Online (3) are not allowed.`,
        };
      }
    }
    ...
    if (cat === 3) {
      ...
      // CHANGE: Owner share is calculated from netPayable (includes GST, minus TDS)
      let ownerShare = 0;
      ...
      if (Math.abs(splitTotal - ownerShare) > 1) {
        return {
          valid: false,
          message: `Owner "${owner.name}": cashAmount (${cashAmt}) + onlineAmount (${onlineAmt}) = ${splitTotal} does not match owner share amount ${ownerShare} (based on net payable including GST)`,
        };
      }
    }
  }
...
// VALIDATE LAND OWNERS SHARE
//
// Owner split is based on netPayable:
//   netPayable = (totalRentalAmount + gstAmount) - tdsAmount
//
// TDS is applied on the GST-inclusive amount (totalRentalAmount + gstAmount).
//
//   typeShare=1 → ownerShare = netPayable * sharePercentage / 100
//   typeShare=2 → ownerShare = shareAmount (fixed, as entered)
//
// Sum of all owner shares must equal netPayable (±1 tolerance)
//
// Example: totalRentalAmount=50,000 | GST 18% → gstAmount=9,000
//          totalWithGst = 59,000 | TDS 10% → tdsAmount=5,900
//          netPayable = 59,000 - 5,900 = 53,100
//
//   Single owner (100%)         → shareAmount = 53,100
//   Two owners   (60% + 40%)    → 31,860 + 21,240 = 53,100
// ─────────────────────────────────────────────────────────────
const validateLandOwnerShares = (
  landOwners,
  totalRentalAmount,
  ...
  // Step 2 — TDS base is GST-inclusive: (totalRentalAmount + gstAmount)
  // Example: 50,000 + 9,000 = 59,000 → TDS 10% = 5,900
  const totalWithGst = parseFloat((totalRentalAmount + gstAmount).toFixed(2));

  const tdsAmount =
    tdsApplicable === 1 && tdsPercentage > 0
      ? parseFloat(((totalWithGst * tdsPercentage) / 100).toFixed(2))
      : 0;

  // Step 3 — netPayable = (totalRentalAmount + GST) - TDS
  // 59,000 - 5,900 = 53,100
  const netPayable = parseFloat(
    (totalWithGst - tdsAmount).toFixed(2)
  );

  // Step 4 — Sum owner shares (against netPayable)
  let totalComputedAmount = 0;
  ...
  for (const owner of landOwners) {
    ...
    if (typeShare === 1) {
      ...
      // CHANGE: percentage applied to netPayable (was: netPayable of base only)
  ...
  if (diff > 1) {
    return {
      valid: false,
      message: `Total owner share (${totalComputedAmount.toFixed(2)}) does not match net payable amount (${netPayable.toFixed(2)}) [(${totalRentalAmount} + GST ${gstAmount}) - TDS ${tdsAmount}]. Difference: ${diff.toFixed(2)}`,
    };
  }
...
const mediaOnboarding = async (req, res) => {
  try {
    ...
    if (mediaData.landOwners?.length === 1) {
      ...
      if (!owner.typeShare) {
        owner.typeShare = 1;         // percentage-based
        owner.sharePercentage = 100; // 100% of netPayable (includes GST)
      }
    }
    ...
    // CHANGE: Share validation now uses netPayable (totalRentalAmount + GST - TDS)
    // as the base amount that all owner shares must sum to.
    if (mediaData.landOwners?.length && mediaData.rentalPayment?.totalRentalAmount) {
      const tdsApplicable = Number(mediaData.rentalPayment.tdsApplicable) || 0;
      const envTdsPercent = parseFloat(process.env.TDS_PERCENTAGE || "0");
      const tdsPercentage =
      ...
      const shareCheck = validateLandOwnerShares(
        ...
        tdsPercentage,
        gstApplicable
        // gstType no longer needed — GST is always added to netPayable
      );
      ...
      // ── VALIDATION: Payment category per owner ─────────────
      // CHANGE: passes gstApplicable so online-only rule can be enforced
      const pmCatCheck = validateOwnerPaymentCategories(
        mediaData.landOwners,
        shareCheck.netPayable,
        gstApplicable           // ← NEW PARAM
      );
      if (!pmCatCheck.valid) {
        return errorResponse(res, pmCatCheck.message, null, 400);
      }
    ...
    if (id) {
      ...
      // Reload from DB to get all pre-save computed values
      media = await MediaOnboarding.findById(media._id).lean();
...
//     if (mediaType) {
//       filter.mediaType = Array.isArray(mediaType) ? { $in: mediaType } : mediaType;
//     }
//     if (status) {
...
//     const mediaListData = await MediaOnboarding.find(filter)
//       .sort({ createdAt: -1 })
//       .skip((pageNumbers - 1) * pageSize)
//       .limit(pageSize)
//       .lean();

//     const allData = await MediaOnboarding.find({}, "city mediaType status").lean();

//     const cityFilter = [...new Set(allData.map((item) => item.city))].filter(Boolean);
//     const mediaTypeFilter = [...new Set(allData.map((item) => item.mediaType))].filter(Boolean);
//     const statusFilter = [...new Set(allData.map((item) => item.status))].filter(Boolean);
...
const mediaList = async (req, res) => {
  try {
    const { pageNumber = 1, count = 10, mediaType, city, status, search } = req.body;
    ...
    const combinedFilter =
      Object.keys(searchFilter).length > 0
        ? { $and: [searchFilter, ...(Object.keys(filter).length > 0 ? [filter] : [])] }
        : filter;
