...
exports.createLedgerEntry = async (req, res) => {
  try {
    ...
    if (Array.isArray(req.body)) {
      return handleMultiBatchLedgerEntry(req, res);
    }
    // ✅ Existing bulk flow (single batch with payments[] at top level)
...
/**
 * ✅ NEW — Internal logic for processing a single batch of bulk payments.
 * Extracted from runBulkLedgerEntry to support multi-batch processing.
 */
async function executeBulkPaymentBatch(req, batchData) {
  const { siteBillMode, entryType, payments, date } = batchData;

  // 1) Validation
  if (![1, 2].includes(Number(siteBillMode))) {
    throw { message: "siteBillMode must be 1 (single bill) or 2 (separate)", statusCode: 400 };
  }
  if (!["rental", "gst"].includes(entryType)) {
    throw { message: `entryType must be "rental" or "gst"`, statusCode: 400 };
  }
  if (!Array.isArray(payments) || payments.length === 0) {
    throw { message: "payments must be a non-empty array", statusCode: 400 };
  }

  for (let i = 0; i < payments.length; i++) {
    if (!["current", "pastCycle", "outstanding"].includes(payments[i].targetType)) {
      throw { message: `payments[${i}].targetType must be "current", "pastCycle", or "outstanding"`, statusCode: 400 };
    }
    if (!payments[i].paymentMode || !["Cash", "Online"].includes(payments[i].paymentMode)) {
      throw { message: `payments[${i}].paymentMode must be "Cash" or "Online"`, statusCode: 400 };
    }
    if (payments[i].paymentMode === "Online" && !payments[i].utrNumber) {
      throw { message: `payments[${i}].utrNumber is required when paymentMode is Online`, statusCode: 400 };
    }
  }
  ...
  for (const mId of Object.keys(paymentsByMedia)) {
    ...
    try {
      ...
      for (const item of sitePayments) {
        ...
        if (entryType === "rental" && (targetType === "current" || targetType === "pastCycle")) {
          ...
          if (targetType === "current") {
            media.ledger.push({
              ...
              date: entryDate, status: 1, month: dueMonthLabel, cycle: cycleDateVal, rentalDueId: item.rentalDueId,
          ...
          const parsed = parseDueMonthLabel(dueMonthLabel);
          const bYear = parsed ? String(parsed.year) : String(new Date(cycleDateVal).getUTCFullYear());
          const bMonth = parsed ? MONTH_NAMES[parsed.monthIdx] : MONTH_NAMES[new Date(cycleDateVal).getUTCMonth()];
          ...
          mBucket.entries.push({
            ...
            netPayable: rentalDue.netPayable, rentalDueId: item.rentalDueId, withGst: effectiveWithGst, month: dueMonthLabel, cycle: cycleDateVal,
            paymentMode: item.paymentMode, utrNumber: item.utrNumber, status: 1, date: entryDate, amount, updatedBy, updatedAt: nowIST(),
            isUtrEntry: false, index: targetType === "current" ? media.ledger.length - 1 : null
          });
          ...
          // ✅ Record outstanding rent in history bucket too
          const parsed = parseDueMonthLabel(dueMonthLabel);
          if (parsed) {
            const bYear = String(parsed.year);
            const bMonth = MONTH_NAMES[parsed.monthIdx];
            let yBucket = (media.ledgerHistory || []).find(y => y.year === bYear);
            if (!yBucket) { media.ledgerHistory.push({ year: bYear, months: [] }); yBucket = media.ledgerHistory[media.ledgerHistory.length - 1]; }
            let mBucket = yBucket.months.find(m => m.month === bMonth);
            if (!mBucket) { yBucket.months.push({ month: bMonth, entries: [] }); mBucket = yBucket.months[yBucket.months.length - 1]; }
            ...
            mBucket.entries.push({
              landOwnerId: owner._id, landOwnerName: owner.name, mediaName: media.mediaName, paymentFrequency: media.rentalPayment.paymentFrequency,
              netPayable: amount, rentalDueId: null, withGst: 2, month: dueMonthLabel, cycle: null,
              paymentMode: item.paymentMode, utrNumber: item.utrNumber, status: 1, date: entryDate, amount, updatedBy, updatedAt: nowIST(),
              isUtrEntry: false
            });
            media.markModified("ledgerHistory");
        ...
        else if (entryType === "gst" && (targetType === "current" || targetType === "pastCycle")) {
          if (!item.rentalDueId) throw new Error("rentalDueId required");
          const rentalDue = media.rentalDue?.find(d => String(d._id) === String(item.rentalDueId));
          if (!rentalDue) throw new Error("rentalDue record not found");
          ...
          mBucket.entries.push({
            landOwnerId: owner._id, landOwnerName: owner.name, mediaName: media.mediaName, paymentFrequency: media.rentalPayment.paymentFrequency,
            netPayable: rentalDue.netPayable, rentalDueId: item.rentalDueId, withGst: 1, month: dueMonthLabel, cycle: cycleDateVal,
            paymentMode: item.paymentMode, utrNumber: item.utrNumber, status: 1, date: entryDate, amount, updatedBy, updatedAt: nowIST(),
            isUtrEntry: true
          });
          media.markModified("ledgerHistory");
          ...
          // ✅ Record outstanding GST in history bucket too
          const parsed = parseDueMonthLabel(dueMonthLabel);
          if (parsed) {
            const bYear = String(parsed.year);
            const bMonth = MONTH_NAMES[parsed.monthIdx];
            ...
            mBucket.entries.push({
              landOwnerId: owner._id, landOwnerName: owner.name, mediaName: media.mediaName, paymentFrequency: media.rentalPayment.paymentFrequency,
              netPayable: 0, rentalDueId: null, withGst: 1, month: dueMonthLabel, cycle: null,
              paymentMode: item.paymentMode, utrNumber: item.utrNumber, status: 1, date: entryDate, amount, updatedBy, updatedAt: nowIST(),
              isUtrEntry: true
            });
            media.markModified("ledgerHistory");
  ...
  const distinctMonths = [...new Set(savedEntries.filter(e => e.status === "saved").map(e => e.dueMonth).filter(Boolean))];
  ...
  return {
    ...
    data: {
      ...
      distinctMonthsCovered: distinctMonths, sitesSucceeded, sitesFailed
    }
  };
}

/**
 * ✅ NEW — Handler for multiple payment batches in a single array.
 */
async function handleMultiBatchLedgerEntry(req, res) {
  try {
    const batches = req.body;
    const batchResults = [];
    let totalAmountSavedOverall = 0;

    for (let i = 0; i < batches.length; i++) {
      try {
        const result = await executeBulkPaymentBatch(req, batches[i]);
        batchResults.push({ batchIndex: i, entryType: batches[i].entryType, status: "success", ...result });
        totalAmountSavedOverall += result.data.totalAmountSaved;
      } catch (batchError) {
        batchResults.push({
          batchIndex: i, entryType: batches[i].entryType, status: "failed",
          message: batchError.message || "Error processing batch",
          statusCode: batchError.statusCode || 500
        });
      }
    }

    const failedCount = batchResults.filter(b => b.status === "failed").length;
    const partialCount = batchResults.filter(b => b.statusCode === 207).length;

    return successResponse(res, failedCount === 0 ? "All payment batches processed successfully" : `Processed with ${failedCount} batch failure(s)`, {
      overallSummary: {
        totalBatchesProcessed: batches.length,
        batchesSucceeded: batches.length - failedCount,
        batchesFailed: failedCount,
        totalAmountSaved: totalAmountSavedOverall
      },
      batchResults
    }, (failedCount === 0 && partialCount === 0) ? 201 : 207);
  } catch (error) {
    console.error("handleMultiBatchLedgerEntry error:", error);
    return errorResponse(res, "Something went wrong while processing multiple batches", { error: error.message }, 500);
  }
}

async function runBulkLedgerEntry(req, res) {
  try {
    const result = await executeBulkPaymentBatch(req, req.body);
    return successResponse(res, result.message, result.data, result.statusCode);
  } catch (error) {
    console.error("runBulkLedgerEntry error:", error);
    ...
    function hasAnyPastMonthPending(mediaObj, explicitCutoff) {
      ...
      (mediaObj.ledgerHistory || []).forEach((yearBucket) => {
        (yearBucket.months || []).forEach((monthBucket) => {
          ...
          // ✅ REFACTORED — identify Rent entries by isUtrEntry (fallback to status if missing)
          const rentEntries = entries.filter((e) => e.isUtrEntry !== true && e.paymentMode);
    ...
    const mediaListData = results.map((media) => {
      ...
      if (requestedMonthRange) {
        ...
        if (isSiteCurrentLiveCycleMonth) {
          gst2SourceEntries = (mediaObj.ledger || []).filter((e) => e && e.isUtrEntry !== true);
          gst1SourceEntries = (mediaObj.withGst1Ledger || []).filter((e) => e && e.isUtrEntry === true);
        } else {
          const allMonthEntries = monthBucket?.entries || [];
          // ✅ REFACTORED — separation by entry type (Rent vs GST) rather than withGst value
          gst2SourceEntries = allMonthEntries.filter((e) => e.isUtrEntry !== true);
          gst1SourceEntries = allMonthEntries.filter((e) => e.isUtrEntry === true);
        }
      } else {
        gst2SourceEntries = (mediaObj.ledger || []).filter((e) => e && e.isUtrEntry !== true);
        gst1SourceEntries = (mediaObj.withGst1Ledger || []).filter((e) => e && e.isUtrEntry === true);
      ...
      if (isPendingFilter && earliestPendingMonthKey) {
        ...
        // ✅ REFACTORED — separation by entry type (Rent vs GST) rather than withGst value
        latestLedger = monthEntries.filter((e) => e.isUtrEntry !== true);
        withGst1Ledger = monthEntries.filter((e) => e.isUtrEntry === true);
      } else {
...
latestLedger = latestLedger.sort((a, b) => {
  ...
  return 0;
      ...
      const isGstApplicableForOwner = (owner) => {
        ...
        const owners = mediaObj.landOwners || [];
        const ownerGstAny = owners.some((o) => Number(o.gstApplicable) === 1 || Number(o.gstAmount || 0) > 0);
        const gstFlag = Number(mediaObj.gstApplicableFlag || 0);

        if (ownerGstAny || gstFlag === 2) {
          return Number(owner.gstApplicable) === 1 || Number(owner.gstAmount || 0) > 0;
        }

        if (Number(owner.gstApplicable) === 0 && owner.gstApplicable !== undefined) {
          return false;
        }
      ...
      if (expectedGstPerCycleTotal > 0) {
        autoDueCycles.forEach((cycleDate) => {
          ...
          (mediaObj.landOwners || []).forEach((owner) => {
            ...
            if (gstFlag === 0) {
              const siteGst = Number(mediaObj.rentalPayment?.gstApplicable) === 1;
              const anyOwnerGst = (mediaObj.landOwners || []).some(o => Number(o.gstApplicable) === 1 || Number(o.gstAmount || 0) > 0);
              if (anyOwnerGst) gstFlag = 2;
              else if (siteGst) gstFlag = 1;
            }
    ...
    allTimeCycles.forEach((cycleDate) => {
      ...
      matchingLandOwners.forEach((owner) => {
        ...
        if (!hasEntry) {
          ...
          if (gstFlag === 0) {
            const siteGst = Number(media.rentalPayment?.gstApplicable) === 1;
            const anyOwnerGst = (media.landOwners || []).some(o => Number(o.gstApplicable) === 1 || Number(o.gstAmount || 0) > 0);
            if (anyOwnerGst) gstFlag = 2;
            else if (siteGst) gstFlag = 1;
          }
        ...
        .map((owner) => {
          ...
          if (gstFlag === 0) {
            ...
            const ownerGstArr = (media.landOwners || []).some(
              (o) => Number(o.gstApplicable) === 1 || Number(o.gstAmount || 0) > 0,
            );
            if (ownerGstArr) gstFlag = 2;
            else if (siteGst) gstFlag = 1;
          }
