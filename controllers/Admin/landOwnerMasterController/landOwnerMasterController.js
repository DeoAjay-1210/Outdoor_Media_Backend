...
const { computeOutstandingSummary,dedupeGstBalanceHistory,getOwnerWiseOutstanding   } = require("../../../controllers/Admin/MediaOnboardingController/LedgerNew2Controller")
// ─────────────────────────────────────────────────────────────
...
const upsertLinkedSite = (landOwnerMaster, mediaInfo, owner) => {
  ...
  if (existingIdx !== -1) {
    ...
    site.netPayableToOwner = Number(owner.netPayableToOwner ?? site.netPayableToOwner ?? 0); // ← ADDED
    site.updatedAt = now;
  } else {
...
//         const liveLedgerEntries = Array.isArray(mediaDoc.ledger) ? mediaDoc.ledger : [];
...
//           const ownerId = String(entry.landOwnerId);
...
//           const targetOwnerIds = rowOwnerId ? [rowOwnerId] : ownerIdsOnSite;
...
//             } else if (isPastMonth && !row.isPaid) {
...
//               });
//             }
//             ledgerDataByMediaOwner.get(key).pastGst += amt;
//           });
//         });
...
//       const merged = { currentMonthEntries: [], pastMonthEntries: [], currentGst: 0, pastGst: 0 };
...
//         if (
//           site.updatedAt &&
//           (!latestActivityAt || new Date(site.updatedAt) > new Date(latestActivityAt))
//         ) {
...
//       if (includeCurrentLedger) {
//         entryPayload.currentMonthLedgerEntries = combinedLedger.currentMonthEntries;
//       }
//       if (includePastLedger) {
//         entryPayload.pastMonthLedgerEntries = combinedLedger.pastMonthEntries;
//       }
//       if (includeCurrentGst || includePastGst) {
//         entryPayload.gstSummary = {};
//         if (includeCurrentGst) entryPayload.gstSummary.currentGst = combinedLedger.currentGst;
...
//         if (
//           ownerUpdatedAt &&
//           (!latestActivityAt || new Date(ownerUpdatedAt) > new Date(latestActivityAt))
//         ) {
  ...
  try {
    ...
    if (needsLedgerFields) {
      relatedMediaDocs.forEach((mediaDoc) => {
        ...
        const ownerWiseOutstanding = getOwnerWiseOutstanding(
          mediaDoc,
        ...
        gstBalanceHistory.forEach((row) => {
          ...
          const rowOwnerId = row.ownerId ? String(row.ownerId) : null;
          const targetOwnerIds = rowOwnerId ? [subToMasterMap[rowOwnerId] || rowOwnerId] : ownerIdsOnSite;
          ...
          targetOwnerIds.forEach((ownerId) => {
            ...
            const isGstPaid = (row.isPaid === true || row.isPaid === "true") ||
        ...
        const resolveGstForOwnerThisCycle = (ownerId) => {
          ...
          const rentalGst = (Number(mediaDoc.rentalPayment?.gstApplicable) === 1)
            ? (Number(mediaDoc.rentalPayment?.gstAmount || 0) || Math.round((Number(mediaDoc.rentalPayment?.totalRentalAmount || 0) * envGstPct) / 100))
            : 0;

          const owner = (mediaDoc.landOwners || []).find(
            (o) => o.landOwnerMasterId && String(o.landOwnerMasterId) === ownerId,
          );
          const ownerGst = (owner && Number(owner.gstApplicable) === 1)
        ...
        rentalDueEntries.forEach((due) => {
          ...
          const hasGstRowAlready = gstBalanceHistory.some(
            (g) => String(g.dueMonth || "").trim().toLowerCase() === String(due.dueMonth || "").trim().toLowerCase(),
          );
          if (hasGstRowAlready) return; // already counted above
        ...
        rentalDueEntries.forEach((due) => {
          ...
          (mediaDoc.landOwners || []).forEach((owner) => {
            ...
            requiredModes.forEach((mode) => {
              ...
              const keySubDoc = (mediaDoc.landOwners || []).find(lo => String(lo.landOwnerMasterId) === ownerId)?._id;
              const keySub = keySubDoc ? `${mediaId}_${String(keySubDoc)}` : null;

              let bucket = ledgerDataByMediaOwner.get(keyMaster) || (keySub ? ledgerDataByMediaOwner.get(keySub) : null);

              // ✅ Robust check: look for any entry matching month and mode
              const alreadyCoveredByRealEntry = bucket?.pastMonthEntries.some(
                (e) => String(e.paymentMode).toLowerCase() === String(mode).toLowerCase() &&
                  String(e.dueMonth || "").trim().toLowerCase() === String(due.dueMonth || "").trim().toLowerCase(),
              );
              if (alreadyCoveredByRealEntry) return;
        ...
        Object.entries(ownerWiseOutstanding).forEach(([subOwnerId, amounts]) => {
    ...
    const getLedgerBucketsForSite = (mediaId, ownerIds) => {
      ...
      ownerIds.forEach((ownerId) => {
        ...
        merged.currentMonthEntries.push(...bucket.currentMonthEntries);
        merged.pastMonthEntries.push(...bucket.pastMonthEntries);
        merged.currentPendingRentEntries.push(...bucket.currentPendingRentEntries);
        merged.pastRentalPendingEntries.push(...bucket.pastRentalPendingEntries);
        merged.currentGst += bucket.currentGst;
        merged.pastGst += bucket.pastGst;
    ...
    relatedMediaDocs.forEach((mediaDoc) => {
      ...
      const calcRentalGst = () => Math.round((Number(mediaDoc.rentalPayment?.totalRentalAmount || 0) * envGstPct) / 100);
      const calcOwnerGst = (o) => {
        const pc = Number(o.paymentCategory || 1);
        let base = 0;
        if (pc === 2) base = Number(o.shareAmount || 0);
      ...
      const currentCycle = (rentalDue || []).find(due => {
        const parsed = parseSiteFilterDueMonthLabel(due.dueMonth);
        return parsed && parsed.year === referenceYear && parsed.monthIdx === referenceMonthIdx;
      });

      if (currentCycle) {
        if (Number(currentCycle.gstAmount) > 0) {
          cycleGstAmount = Number(currentCycle.gstAmount);
        } else if (Number(currentCycle.withGst) === 1 || Number(currentCycle.withGst) === 2) {
      ...
      const rentalGst = rentalLevelGstAmount || (Number(mediaDoc.rentalPayment?.gstApplicable) === 1 ? calcRentalGst() : 0);
      const ownersGst = ownerLevelGstTotal || ownersOnThisSite.reduce((sum, o) => {
      ...
      let resolvedSiteGstAmount = Math.max(rentalGst, ownersGst, cycleGstAmount);

      // If still 0 but we have evidence from siteGstFlag or any cycle flag, force calculation
      if (resolvedSiteGstAmount === 0) {
        if (siteGstFlag === 1 || Number(mediaDoc.rentalPayment?.gstApplicable) === 1) {
          resolvedSiteGstAmount = calcRentalGst();
        } else if (siteGstFlag === 2 || ownersOnThisSite.some(o => Number(o.gstApplicable) === 1)) {
          resolvedSiteGstAmount = ownersOnThisSite.reduce((sum, o) => {
            return sum + (Number(o.gstApplicable) === 1 ? (Number(o.gstAmount || 0) || calcOwnerGst(o)) : 0);
          }, 0);
        }
      }
        ...
        ownersDetail: ownersOnThisSite.map((o) => {
          ...
          if (oGst === 0 && resolvedSiteGstAmount > 0) {
            ...
            if (siteGstFlag === 1 || siteGstFlag === 0) {
              oGst = Math.round(resolvedSiteGstAmount / (ownersOnThisSite.length || 1));
    ...
    requestedOwnerIds.forEach((ownerId) => {
      ...
      sites.forEach((site) => {
        ...
        if (needsLedgerFields) {
          ...
          combinedLedger.currentMonthEntries.push(...siteBucket.currentMonthEntries);
          combinedLedger.pastMonthEntries.push(...siteBucket.pastMonthEntries);
          combinedLedger.currentPendingRentEntries.push(...siteBucket.currentPendingRentEntries);
          combinedLedger.pastRentalPendingEntries.push(...siteBucket.pastRentalPendingEntries);
          combinedLedger.currentGst += siteBucket.currentGst;
          combinedLedger.pastGst += siteBucket.pastGst;
          combinedLedger.currentGstPending += siteBucket.currentGstPending;
          combinedLedger.pastGstPending += siteBucket.pastGstPending;
          combinedLedger.currentRentPending += siteBucket.currentRentPending || 0; // ✅ ADDED
      ...
      if (includeCurrentGst || includePastGst || includeCurrentGstPending || includePastGstPending) {
        entryPayload.gstSummary = {};
      ...
      if (includeCurrentPendingRent || includePastRentalPending) {
        ...
        if (includeCurrentPendingRent)
          entryPayload.rentPendingSummary.currentRentPending = combinedLedger.currentRentPending;
        if (includePastRentalPending)
          entryPayload.rentPendingSummary.pastRentPending = combinedLedger.pastRentPending;
      }
      ...
      if (isAnyFilterActive) {
        ...
        const ledgerMatch = needsLedgerFields ? (
          ...
          (includeCurrentGst && combinedLedger.currentGst > 0) ||
          (includePastGst && combinedLedger.pastGst > 0) ||
          (includeCurrentPendingRent && combinedLedger.currentRentPending > 0) ||
          (includePastRentalPending && combinedLedger.pastRentPending > 0) ||
          (includeCurrentGstPending && combinedLedger.currentGstPending > 0) ||
...
//           (includeCurrentGst && combinedLedger.currentGst > 0) ||
//           (includePastGst && combinedLedger.pastGst > 0) ||
//           (includeCurrentPendingRent && combinedLedger.currentPendingRentEntries.length > 0) ||
//           (includePastRentalPending && combinedLedger.pastRentalPendingEntries.length > 0) ||
//           (includeCurrentGstPending && combinedLedger.currentGstPending > 0) ||
//           (includePastGstPending && combinedLedger.pastGstPending > 0)
//         ) : false;
